...
Make sure your system is up to date and your yum or apt-get database is updated:
...
Under RHEL 5.x / Centos 5.x, you must exclude packages from rpmforge repository edit /etc/yum.repos.d/rpmforge.repo and add the line exclude = perl-PathTools*,perl-File-Spec* on the section [rpmforge]
...
RedHat doesn't seem to provide `perl-Net-Telnet perl-XML-Simple perl-SOAP-Lite` packages. PacketFence needs it so we will install it from the rpmforge-extras repository now: 
  ...
  yum install perl-Net-Telnet perl-XML-Simple perl-SOAP-Lite --enablerepo=rpmforge-extras
...
For Debian squeeze, starting with 3.4.0, PacketFence is now providing an deb repository.
...
server, DHCP server, RADIUS server) using:
  ...
  yum groupinstall --enablerepo=PacketFence,rpmforge,of,epel Packetfence-complete
...
Like mentioned at the end of the packages installation, fire up a web browser and go to http://@ip_packetfence:3000/configurator. From there, the configuration process is splited in six (6) distinctive steps, after which you'll have a working PacketFence setup.
...
Introduced in PacketFence 3.6, the outgoing interface should be specified by adding in pf.conf the option interfaceSNAT in inline section. It is a comma limited list of network interfaces like eth0,eth0.100. It's also possible to specify a network that will be routed instead of using NAT by adding in `conf/networks.conf` an option nat=no under one or more network section.
...
Here are the different configuration parameters that can be set for each portal profiles. The only mandatory parameter is "filter", otherwise, PacketFence won't be able to correctly apply the portal profile.
  ...
  guest_modes = the availables guest self-registration modes (sms|email|sponsor)
  ...
  default_auth = the default authentication method that will be selectionned on the captive portal
